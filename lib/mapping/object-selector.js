...
class ObjectSelector {
   ...
   */
  static getForSelect(client, info, allPKsDefined, propertiesInfo, fieldsInfo, orderByColumns, allowFiltering = false) {
    return Promise.all(
      info.tables.map(t => {
        if (t.isView) {
      ...
      .then(tables => {
        for (let i = 0; i < tables.length; i++) {
          ...
          if (![keyMatches.all, keyMatches.some].includes(keysAreIncluded([...table.partitionKeys, ...convertIndexToTableKey(table.indexes)], propertiesInfo))) {
          ...
          if (propertiesInfo.length > table.partitionKeys.length) {
            ...
            if (!allPropertiesArePrimaryKeys && !allowFiltering) {
              continue;
            }
          }
...
function convertIndexToTableKey(indexs) {
  const indox = [];
  
  for (const index of indexs) {
    indox.push({
      name: index.target,
      type: { code: keyMatches.some, info: null, options: { frozen: false } },
      isStatic: false
    });
  }
