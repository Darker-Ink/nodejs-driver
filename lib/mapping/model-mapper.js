...
class ModelMapper {
  ...
  get(doc, docInfo, executionOptions) {
    ...
    const ToTableCasing = this._handler.info._mappings.objectToTableCasing(doc);

    return this._handler.getSelectExecutor(ToTableCasing, docInfo, true)
      .then(executor => executor(ToTableCasing, docInfo, executionOptions))
      .then(result => {
        const First = result.first();

        if (!First) return null;

        return this._handler.info._mappings.objectToFixedCasing(First);
      });
  }
  ...
  find(doc, docInfo, executionOptions) {
    ...
    return this._handler.getSelectExecutor(ToTableCasing, docInfo, false)
      .then(executor => executor(ToTableCasing, docInfo, executionOptions));
  }
  ...
  insert(doc, docInfo, executionOptions) {
    ...
    return this._handler.getInsertExecutor(ToTableCasing, docInfo)
      .then(executor => executor(ToTableCasing, docInfo, executionOptions));
  }
  ...
  update(doc, docInfo, executionOptions) {
    ...
    return this._handler.getUpdateExecutor(ToTableCasing, docInfo)
      .then(executor => executor(ToTableCasing, docInfo, executionOptions));
  }
  ...
  remove(doc, docInfo, executionOptions) {
    ...
    return this._handler.getDeleteExecutor(ToTableCasing, docInfo)
      .then(executor => executor(ToTableCasing, docInfo, executionOptions));
  }
