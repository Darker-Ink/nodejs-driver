...
class MappingHandler {
  ...
  getSelectExecutor(doc, docInfo, allPKsDefined) {
    ...
    const allowFiltering = (docInfo && docInfo?.allowFiltering) ?? false;

    return this._client.connect()
      .then(() =>
        ObjectSelector.getForSelect(this._client, this.info, allPKsDefined, propertiesInfo, fieldsInfo, orderByColumns, allowFiltering))
      .then(tableName => {
        // Part of the closure
        const query = QueryGenerator.getSelect(tableName, this.info.keyspace, propertiesInfo, fieldsInfo,
          orderByColumns, limit, allowFiltering);
  ...
  getSelectAllExecutor(docInfo) {
    ...
    const query = QueryGenerator.getSelect(
      tableName, this.info.keyspace, utils.emptyArray, fieldsInfo, orderByColumns, limit, allowFiltering);
